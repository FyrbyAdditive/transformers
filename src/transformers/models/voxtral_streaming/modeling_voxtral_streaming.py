...
from .configuration_voxtral_streaming import (
    VoxtralStreamingConfig,
    VoxtralStreamingEncoderConfig,
    VoxtralStreamingTextConfig,
)
...
@use_kernelized_func(apply_rotary_pos_emb)
class VoxtralStreamingTextAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: VoxtralStreamingTextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.head_dim = getattr(config, "head_dim", None) or config.hidden_size // config.num_attention_heads
...
class VoxtralStreamingTextMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
@use_kernel_forward_from_hub("RMSNorm")
class VoxtralStreamingTextRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        VoxtralStreamingTextRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
        self.weight = nn.Parameter(torch.ones(hidden_size))
        self.variance_epsilon = eps
...
class VoxtralStreamingTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = VoxtralStreamingTextAttention(config=config, layer_idx=layer_idx)
        self.mlp = VoxtralStreamingTextMLP(config)
        self.input_layernorm = VoxtralStreamingTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = VoxtralStreamingTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class VoxtralStreamingTextPreTrainedModel(PreTrainedModel):
    config: VoxtralStreamingTextConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["VoxtralStreamingTextDecoderLayer"]
    _skip_keys_device_placement = ["past_key_values"]
    _supports_flash_attn = True
    _supports_sdpa = True
    _supports_flex_attn = True
    ...
    _supports_attention_backend = True
    _can_record_outputs = {
        "hidden_states": VoxtralStreamingTextDecoderLayer,
        "attentions": VoxtralStreamingTextAttention,
    }


class VoxtralStreamingTextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: VoxtralStreamingTextConfig, device=None):
        super().__init__()
        self.max_seq_len_cached = config.max_position_embeddings
        self.original_max_seq_len = config.max_position_embeddings
    ...
    @staticmethod
    def compute_default_rope_parameters(
        config: VoxtralStreamingTextConfig | None = None,
        device: Optional["torch.device"] = None,
        seq_len: int | None = None,
    ) -> tuple["torch.Tensor", float]:
        """
...
@auto_docstring
class VoxtralStreamingTextModel(VoxtralStreamingTextPreTrainedModel):
    def __init__(self, config: VoxtralStreamingTextConfig):
        super().__init__(config)
        ...
        self.layers = nn.ModuleList(
            [VoxtralStreamingTextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = VoxtralStreamingTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = VoxtralStreamingTextRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
...
@auto_docstring
class VoxtralStreamingTextForCausalLM(VoxtralStreamingTextPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = VoxtralStreamingTextModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, VoxtralStreamingTextForCausalLM

        >>> model = VoxtralStreamingTextForCausalLM.from_pretrained("meta-voxtral_streaming_text/VoxtralStreamingText-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-voxtral_streaming_text/VoxtralStreamingText-2-7b-hf")
...
__all__ = ["VoxtralStreamingForConditionalGeneration", "VoxtralStreamingEncoder", "VoxtralStreamingTextForCausalLM"]
